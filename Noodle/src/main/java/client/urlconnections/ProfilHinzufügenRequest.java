...
public class ProfilHinzufügenRequest {
    ...
    public ProfilHinzufügenRequest (String charset, User user, String passwort) throws IOException {
    ...
    protected String getURL(User user) {
        String ret = "http://localhost:1337/addProfile";
    ...
    public static int addProfile(User user, String passwort) {
        int id = -1;
        try {
            ProfilHinzufügenRequest request = new ProfilHinzufügenRequest("UTF-8", user, passwort);
            id = request.finish();
        } catch (IOException e) {
            e.printStackTrace();
        }
