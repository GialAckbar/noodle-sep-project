...
public class KursListe {
    ...
    @FXML private TextField courseNameAll;
...
//        Zur Kursansicht wechseln
    ...
    @FXML
    void goToAttended() { //Methode zum übergeben der aktuellen Suchleiste, wenn von globalen Kursliste zur belegten gewechselt wird
        //Aktuellen Inhalt der Suchleiste übergeben
        BelegteKursListe.changedInput = courseNameAll.getText();
    ...
    void searchCourse(String input) {
            ...
            if(lehrVList.size() == 0) {
                String unavInput = courseNameAll.getText();
                Group t = new Group();
    ...
    private void initialize() {
        Response response = Launcher.requestHandler.request(RequestHandler.Art.GET, "/kursliste/NAONLY", LehrveranstaltungsListe[].class);

        if(response.statusCode == 200) {
            LehrveranstaltungsListe[] lvL = (LehrveranstaltungsListe[]) response.getElement();
            list= new ArrayList<>(Arrays.asList(lvL));
            courseNameAll.getText();
 ...
 //       Übernahme des bisher gesuchten Inputs falls vorhanden
        if(BelegteKursListe.changedInput != null) {
            courseNameAll.setText(BelegteKursListe.changedInput);
            searchCourse(BelegteKursListe.changedInput);
        }
        courseNameAll.textProperty().addListener((ov, v, v1) -> searchCourse(v1));
    }
    ...
    //Kurseintrag
    private GridPane addGridEntry(String bss, String ccc, String daten, int id) {
        ...
        GridPane.setMargin(course, new Insets(0,0,0,300));

        //Linie
        Line l = new Line(2390, 0, 1326, 0);
    ...
    private GridPane addSemesterGridEntry(String semester, int jahr) {
